-- Hunter AI Schema Update - Fixed Version
-- This script safely adds missing columns and tables in the correct order
...
DO $$
BEGIN
    -- Only create indexes if the corresponding tables exist
    ...
    -- Articles indexes (now that we've confirmed columns exist)
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content_queue') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='content_queue' AND column_name='scheduled_for') THEN
...
-- Step 4: Insert sample data only if tables are empty
DO $$
BEGIN
    ...
    -- Insert content queue sample data
