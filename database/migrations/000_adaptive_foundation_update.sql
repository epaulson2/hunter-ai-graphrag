-- Hunter AI Schema Discovery and Safe Update
-- This script first checks your existing schema and adapts to it

-- Step 1: Add missing columns to existing articles table
DO $$ 
BEGIN
...
-- Step 4: Enhance existing content_queue table with missing columns
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content_queue') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='content_queue' AND column_name='content_preview') THEN
            ALTER TABLE content_queue ADD COLUMN content_preview TEXT;
        END IF;
        
        -- Add source_type if missing
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='content_queue' AND column_name='source_type') THEN
            ALTER TABLE content_queue ADD COLUMN source_type VARCHAR(100) DEFAULT 'manual';
        END IF;
        
        -- Add priority if missing
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='content_queue' AND column_name='priority') THEN
            ALTER TABLE content_queue ADD COLUMN priority INTEGER DEFAULT 5;
        END IF;
        
        -- Add status if missing
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='content_queue' AND column_name='status') THEN
            ALTER TABLE content_queue ADD COLUMN status VARCHAR(50) DEFAULT 'pending';
        END IF;
        
        -- Add scheduled_for if missing
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='content_queue' AND column_name='scheduled_for') THEN
            ALTER TABLE content_queue ADD COLUMN scheduled_for TIMESTAMP WITH TIME ZONE;
        END IF;
        
        -- Add processed_at if missing
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='content_queue' AND column_name='processed_at') THEN
            ALTER TABLE content_queue ADD COLUMN processed_at TIMESTAMP WITH TIME ZONE;
        END IF;
    ELSE
        -- Create content_queue table if it doesn't exist at all
...
BEGIN
    ...
    -- Articles indexes
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'articles') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='articles' AND column_name='article_type') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content_queue') THEN
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='content_queue' AND column_name='status') AND
           EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='content_queue' AND column_name='priority') THEN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='content_queue' AND column_name='scheduled_for') THEN
...
-- Step 6: Insert sample data only if tables exist and have the required columns
DO $$
BEGIN
    -- Insert businesses sample data
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'businesses') THEN
    ...
    -- Insert content queue sample data only if all required columns exist
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content_queue') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='content_queue' AND column_name='content_preview') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='content_queue' AND column_name='source_type') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='content_queue' AND column_name='priority') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='content_queue' AND column_name='status') THEN
